...
class Test_validate( unittest.TestCase ):
    ...
    def test_should_no_accept_less_to_9_chars( self ):
        with self.assertRaises( ValidationError ):
            self.adapter.validate( ssn='12345678' )

    def test_should_no_accept_less_to_9_chars_asdf( self ):
        with self.assertRaises( ValidationError ):
            self.adapter.validate( ssn='1234567890' )

    def test_if_send_more_data_not_care( self ):
        result = self.adapter.validate( ssn='123456789', asdf='helllo' )
        self.assertDictEqual( result, { 'ssn': '123456789' } )
